...
Partial Class GF_incProvisionProcess
  ...
  Private ShowPopup As enumVoucherData = enumVoucherData.None
  Protected Sub GVincProvisionProcess_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVincProvisionProcess.RowCommand
    ...
    If e.CommandName.ToLower = "DisplayWF".ToLower Then
      Try
        SNos = ""
        Dim SerialNo As Int32 = GVincProvisionProcess.DataKeys(e.CommandArgument).Values("SerialNo")
        Dim x As List(Of SIS.TA.vchData) = SIS.INC.incProvisionProcess.GetVchData(SerialNo, 0, 0, True)
        If x.Count > 0 Then
          SNos = SerialNo
          ...
          PostVoucher.DisplayContent.Controls.Add(New LiteralControl(SIS.TA.vchData.GetHTML(x, True)))
          sAmt = x.Sum(Function(t) t.MAmt)
          str = "<h3> MEIS Voucher: " & sAmt & "</h3>"
          PostVoucher.DisplayContent.Controls.Add(New LiteralControl(str))
          PostVoucher.DisplayContent.Controls.Add(New LiteralControl(SIS.TA.vchData.GetHTML(x, False)))
    ...
    If e.CommandName.ToLower = "ProcessWF".ToLower Then
      Try
        SNos = ""
        Dim SerialNo As Int32 = GVincProvisionProcess.DataKeys(e.CommandArgument).Values("SerialNo")
        Dim x As List(Of SIS.TA.vchData) = SIS.INC.incProvisionProcess.GetVchData(SerialNo, 0, 0)
        If x.Count > 0 Then
          SNos = SerialNo
          Dim sAmt As Decimal = x.Sum(Function(t) t.DAmt)
          Dim str As String = "<h3> DBK Voucher: " & sAmt & "</h3>"
    ...
    Try
      ...
      If x IsNot Nothing Then
        If x.Count > 0 Then
          ...
          y = SIS.TA.taVoucher.CreatePostVoucherData(x, vchDt, VchBatch)
        End If
      End If
